...
ButtPlugL = Toy('ButtPlugL', '大号肛塞',
                '你必须经过充分的扩张才能使用，并且插入过程会让你感觉痛苦。')
ButtPlugM = Toy('ButtPlugM', '中号肛塞',
                '经过一点扩张准备就可以使用，并且不会感到疼痛。')
ButtPlugS = Toy('ButtPlugS', '小号肛塞',
                '可以很容易就插入进去')
LockButtPlug = Toy('LockButtPlug', '带锁肛塞',
                   '结构复杂的肛塞，底部有一个小的锁定装置，可以用特殊的钥匙打开。一旦插入，只有拥有钥匙的人才能将其拔出。')
AnalBeadsL = Toy('AnalBeadsL', '大号拉珠', '你必须经过充分的扩张才能使用，并且使用过程会让你感觉痛苦')
AnalBeadsM = Toy('AnalBeadsM', '中号拉珠', '经过一点扩张准备就可以使用，并且不会感到疼痛。')
AnalBeadsS = Toy('AnalBeadsS', '小号拉珠', '可以很容易就插入进去')
EnemaKit = Toy('EnemaKit', '灌肠用具', '可以是注射器、淋浴的水管、灌肠袋或者任何可以让水流进去的东西')
DildoL = Toy('DildoL', '大号假阳具', '你必须经过充分的扩张才能使用，并且插入过程会让你感觉痛苦。')
DildoM = Toy('DildoM', '中号假阳具', '经过一点扩张准备就可以使用，并且不会感到疼痛。')
DildoS = Toy('DildoS', '小号假阳具', '可以很容易就插入进去')
